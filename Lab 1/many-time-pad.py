...
def check_all(arr_idx, ch, char_idx, cts_arrays):
    """Check if the XOR result is valid for all ciphertexts."""
    err_cnt = 0
...
def main():
    # List of ciphertexts (hex-encoded)
    cts = [
    ...
    # Convert all ciphertexts to arrays of integers
    cts_arrays = [convert_to_array(ct) for ct in cts]
    # Initialize plaintext arrays with underscores
    pt = [['_' for _ in range(len(ct))] for ct in cts_arrays]

    # Iterate through all pairs of ciphertexts
